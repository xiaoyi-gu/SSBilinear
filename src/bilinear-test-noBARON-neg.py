...
def main():
    ...
    instance_filename = f"instances/{m}-{n}-{p}-{itr}-{eps_viol}-{ind}-neg.pkl"
    fo = None
    fp = None
    ...
    for _ in range(itr):
        ...
        if fo is None:
            ...
            bi_costs = [[((2 * random.random() - 1) if random.random() < p else 0)
        ...
        pdCSV.append([zMc, zPrev, zGRB, zGRB_bound, zFin, zFin_bound, zOpt,
                        n_MC,
                        1 - zMc / zOpt, 1 - zPrev / zOpt, 1 - zGRB / zOpt, 1 - zGRB_bound / zGRB,
                        1 - zFin / zOpt, 1 - zFin_bound / zFin,
                        (zPrev - zMc) / (zOpt - zMc),
                        m, n, p, eps_viol,
                        tGRB, tMc, tFin,
                        t_grbnlp, t_heuris,
    ...
    pdDF = pd.DataFrame(pdCSV, columns=["zMc", "zMin-heuristic", "zGRB", "zGRB_bound",
                                        ...
                                        "nMC",
                                        "gapMc", "gapMin-heuristic", "gapGRB", "gapGRB_raw",
                                        "gapFin", "gapFin_raw",
                                        "gapClosed-heuristic",
                                        "m", "n", "p", "eps-viol", 
